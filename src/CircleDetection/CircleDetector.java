...
public class CircleDetector {
    ...
    public static ReturnCircle detectCircles(BufferedImage img) {
    ...
    public static MatOfPoint pointFeaturesToCircleCenter(Mat image, Point center) {
        ...
        //will identify points in the autoControllerImage
        Imgproc.goodFeaturesToTrack(gray, corners, 100, 0.1, 0.0);
        Point[] cornerpoints = corners.toArray();

        int x = 1;
        for (Point points : cornerpoints) {
            //draw vectors from identified autoControllerImage features to the circle center
            Imgproc.line(image, points, center, new Scalar( 0, 255, 0, 128 ) );
        }

        //draw corners

        //rectangle(autoControllerImage, new Point(cornerpoints[0].x, cornerpoints[0].y), new Point(cornerpoints[1].x, cornerpoints[1].y), new Scalar(255, 255, 0));
